...
function Input({label,value,onChange,name,type='text'}){return <label className="block"><span className="block text-xs font-semibold text-slate-600 mb-1.5">{label}</span><input name={name} type={type} value={value??''} onChange={e=>onChange&&onChange(e.target.value)} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-teal-500" /></label>}
